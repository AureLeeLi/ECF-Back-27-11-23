...
@section('content')
    ...
    <div class="flex">
        <form action="" method="post" class="w-50" enctype="multipart/form-data">
            ...
            <input type="file" name="cover" id="cover" value="{{ old('cover', $item->cover) }}" class="my-2 py-2 px-2 border-2 rounded-lg border-[bg-slate-50] w-full">
            <div class="w-full flex items-center space-x-4">
                <label for="dimensions">Dimensions :</label>
                @foreach($dimensions as $dimension)
                    <input type="radio" id="dimension{{ $dimension->id }}" name="dimension_id" value="{{ $dimension->id }}" 
                        {{ $matelas->dimension_id == $dimension->id ? 'checked' : '' }}>
